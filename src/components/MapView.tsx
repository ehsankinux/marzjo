...
export default function MapView() {
  const { mapContainer, errorMessage } = useMapView();
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {errorMessage ? (
        <div className="absolute top-4 right-4 z-30 max-w-xs rounded-2xl border border-red-200 bg-white/95 p-4 text-sm text-red-600 shadow-lg dark:border-red-500/40 dark:bg-neutral-900/95 dark:text-red-200">
          {errorMessage}
        </div>
      ) : null}
    </div>
...
function LegendRow({
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className={`${colorClass} ${isSquare ? "h-3 w-3 rounded-sm" : "h-3 w-3 rounded-full"} border border-white/60 shadow`}
      />
      <span>{label}</span>
    </div>
  );
